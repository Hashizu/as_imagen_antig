...
def main():
    ...
    parser.add_argument("--keyword", type=str, required=True, help="Main theme keyword")
    parser.add_argument("--tags", type=str, default="", help="Comma-separated mandatory tags")
    parser.add_argument("--n", type=int, default=10, help="Number of variations (ideas)")
    parser.add_argument("--model", type=str, default="gpt-image-1.5", help="OpenAI Image Model")
    parser.add_argument("--size", type=str, default=None, help="Image size (e.g. 1024x1024)")
    parser.add_argument("--quality", type=str, default=None, help="Image quality (standard or hd)")
    parser.add_argument("--response_format", type=str, default=None, help="Response format (url or b64_json)")
    parser.add_argument("--style", type=str, default="japanese_simple", help="Style type")
    ...
    base_output_dir = os.path.join(os.getcwd(), f"output/{timestamp}_{safe_keyword}")
...
"""
    ...
    # 1. アイデア生成
    print("画像のアイデアを生成中...")
    ideas = generator.generate_image_description(args.keyword, n_ideas=args.n, style=args.style)
    ...
    if not csv_data:
        ...
        metadata_mgr.export_csvs(csv_data, upscale_dir) # submit.csv を upscale_dir (ルート) に出力
        
        # prompt.csv を generated_images に出力する必要がある
        import pandas as pd
        prompt_df = pd.DataFrame(csv_data)[['filename', 'prompt']]
        prompt_df.to_csv(os.path.join(images_dir, "prompt.csv"), index=False, encoding='utf-8-sig', errors="ignore") 
